...
function updateSituation(situation: PlanetSituation, planet: Planet) {
  ...
  alert(
    `A situação do planeta ${planet.name} foi atualizada de ${beforeSituation} para ${situation}`
  );
}
...
function thirdMenuOption() { // Esse planet é o objeto do planeta encontrado pela função findPlanet
    promptValidPlanet(planet => { //se planet for encontrado, o bloco de código será executado:
